...
//			driver.switchTo().defaultContent();
		...
		WebElement roundtripButton = driver.findElement(By.xpath("//li[text()[contains(.,'Round')]]/span"));
		...
		while (true) {
			...
			Boolean checker = (check1.contains("Bengaluru") || check2.contains("Bengaluru"));
...
//				System.out.println(checker);
		...
		// handling calendar for round trip

		// the one way date to be selected (dd/mm/yy)

		LocalDate calendar_tobefound = LocalDate.of(2020, Month.AUGUST, 26);
		...
		// the return date for return trip

				LocalDate rcalendar_tobefound = LocalDate.of(2021, Month.FEBRUARY, 20);

				Integer rdateint_tobefound = rcalendar_tobefound.getDayOfMonth();

				String rdate_tobefound = rdateint_tobefound.toString();

				Month rmonthval_tobefound = rcalendar_tobefound.getMonth();

				String rmonth_tobefound = rmonthval_tobefound.name();

				Integer ryearint_tobefound = rcalendar_tobefound.getYear();

				String ryear_tobefound = ryearint_tobefound.toString();
		...
		// selecting desired month and year for outbound
		...
		// selecting day for outbound- Naveen Method using xpath
		outer:
			...
			// selecting desired month and year for inbound

			 leftrightlabel = driver.findElements(By.xpath("//div[@class='DayPicker-Caption']/div"));
			

			 label_desired = rmonth_tobefound + ryear_tobefound;
				...
				for (int i = 1; i <= 5; i++) {
					...
					for (int k = 1; k <= 7; k++) {
						...
						if (driver
							...
							if (cust_xpath.getText().equals(rdate_tobefound)) {
